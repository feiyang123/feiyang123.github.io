...
{
 init();
 var functionitemList = $(".functionitem");
 $("#wd").click(function(){
				$(this).val("");		 
		});
 var functionitemListValue="";
 for(var i=0;i<functionitemList.length;i++)
 {
	 functionitemListValue = functionitemListValue + functionitemList[i].innerHTML + ",";
 }
 //alert(functionitemListValue);
 dearray(functionitemListValue);
 //dearray("asp,csdn,asp.net,php,jsp,dvbbs,baidu,92mk,123cha,hao123,google,3721,123456,popasp,alimama,aku,cool");
 downList(arrList,"wd");
 var SBNT=document.getElementById("Sbnt") // 获取按钮
 SBNT.onclick = seachBnt;
}
function seachBnt()
{
	var seachValue=document.getElementById("wd").value;
	//alert(seachValue);
	$.blockUI(
			{
			fadeIn: 700, 
            fadeOut: 700,
			message:"<div class='showMsg'>"+$(".functionitem").filter(":contains("+seachValue+")").next().html()+"</div>",
			css: {
	...
	$('.blockOverlay').attr('title','点我就消失隐藏').click($.unblockUI); 
	//$('.showMsg').attr('title','点击后隐藏').click($.unblockUI); 
}
...
//出现光标
document.write("<div id=\"keysList\" style=\"position:absolute;display:none;background:#ffffff;border: 1px solid #CCCCCC;font-size:14px;color:#000000;cursor: default;\" onblur> </div>");
